 ...
 * throw on startup if DATABASE_URL isn't set yet, and Proxy-wrapping the
 * client is known to break downstream libraries that introspect it.
 */
let _sql = null;
...
export function getSql() {
  if (!_sql) {
    ...
    if (!url) {
      throw new Error(
        "DATABASE_URL is not set. Add it to .env.local (see README) and restart the API server.",
      );
    }
    _sql = neon(url);
  }
