 ...
 *
 * Server-side and not `localStorage` for the reason `src/lib/activity.js` and
...
/**
 * Where the log file lives, and which of three situations is in effect —
 * "file", "temp" or "memory".
 *
 * `.data/` beside the project by default, so a restart doesn't lose the log.
 * On a read-only deployment filesystem (everywhere but `/tmp`, on most
 * platforms) that `mkdir` throws, and the temp directory is used instead —
 * which survives while the process lives but not a restart. That is the
 * honest ceiling of a no-database setup, and it is why `storageKind()`
 * exists: the caller reports which one is in play rather than letting a
 * deployment quietly look more durable than it is.
 */
let dir = null;
let kind = null;
...
function load() {
  ...
  try {
    const parsed = JSON.parse(readFileSync(join(base, FILE_NAME), "utf8"));
 ...
 *
 * `filters` is `{ managerEmail, staffEmail, leadId, project, from, to }`, all
 * optional and all nullable. `project` is a substring, matched
 * case-insensitively against the label and the lead name — the same two
 * columns the SQL path's `project` filter searches.
 */
