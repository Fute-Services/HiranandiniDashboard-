...
import { portfolioGroups, projectsIn } from "@/data/properties";
...
export function PropertyShowcase({ properties }) {
   ...
   * lib/inventory.js) — a project absent here just shows no urgency note,
  ...
  const openViewer = useCallback((property, within) => {
  ...
  const closeViewer = useCallback((reason) => {
  ...
  useEffect(() => {
    ...
    if (!active) {
      navigate(SESSION_START_PATH, { replace: true });
      return;
    }
    setSession(active);
   ...
   * `KickWatcher` (app shell) so it works on every page, not just this
  ...
  useEffect(() => {
    ...
    if (
      ...
      (blockedSlugs.includes(open.property.slug) ||
        blockedSlugs.includes(open.property.portfolioSlug ?? open.property.slug))
    ) {
   ...
   * in sessionStorage (src/lib/project-time.js) and pauses itself when the
   ...
   * and data/properties.js's rail entries) — so a rail of towers has to ask
  ...
  const portfolioOf = useCallback((property) => property.portfolioSlug ?? property.slug, []);
 ...
 *
 * It carries Log out as well as ×, because this covers the whole screen —
 * including the header the rest of the app signs out from. Without it, ending
 * a presentation from inside a project meant closing back to the showcase
 * first, and on a kiosk with no browser chrome that is the difference between
 * one tap and being stuck. It goes through the parent's `leave`, so the close
 * event, the session's final step time and Sperto's "OUT" are all written
 * exactly as they are from the header (see `leave`).
 */
function ProjectViewer({ property, onClose, onLogOut, leaving }) {
 ...
 */
// Nothing renders this today — the bottom card shelf it was drilled into
// from is gone (see the rail note above). Kept because the panel itself is
// still the answer for floor plan / gallery / amenities / brochure, and
// restoring the drill-down is a render call away.
function PropertyDetailsModal({ property, onClose, onOpenProject }) {
