...
function wrapOffset(i, position, count) {
  const half = count / 2;
  return ((((i - position + half) % count) + count) % count) - half;
}

function faceTransform(offset) {
  return (
...
export function PropertyCarousel({ properties }) {
  ...
  const setAnimated = useCallback((on) => {
  ...
  const render3d = useCallback(() => {
    ...
    const faces = Array.from(prism.children);
  ...
  return (
    <div className={styles.page}>
        ...
        <div ref={scalerRef} className={styles.scaler}>
          <div ref={prismRef} className={styles.prism} data-animated="false">
            {properties.map((property, i) => {
              // Matches what render3d writes at angle 0, so the first paint
              // agrees with the first frame the loop draws.
      ...
      {/* Goes through the app's one sign-out path rather than expiring the
          cookie itself: the auth cookie is httpOnly now, so clearing it from
          here never worked — only /api/logout can. See lib/sign-out.js. */}
      <button
        type="button"
