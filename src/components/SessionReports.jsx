...
import { actorFields, listActivity, track } from "@/lib/activity";
...
function relativeTime(atMs, startedAtMs) {
 ...
 * activity log (src/lib/activity.js) rather than stored as its own record,
 * since the log is the single source of truth admins/managers can't have
 * edited out from under them.
 */
export function groupPresentations(events) {
  const groups = new Map();
  for (const e of events) {
    if (!e.leadId) continue;
    // Reassignment events (see server/routes/leads.js) carry a leadId for
    // filtering but aren't part of any actual customer walkthrough — a
    // synthetic "leads-<id>" session id, not a real login session, keeps them
    // out of the Customer Visits table instead of showing up as a phantom
  ...
  return [...groups.values()]
    .map((evs) => {
      ...
      return {
        ...
        leadId: first.leadId,
        leadName: first.leadName ?? first.leadId,
 ...
 * per click. */
function buildTopProperties(list, max = 6) {
 ...
 * `distinctShown`) so repeatedly toggling the same tab can't inflate it. */
function buildStaffLeaderboard(list) {
  const byStaff = new Map();
  for (const p of list) {
    const row =
      byStaff.get(p.staffEmail) ??
      { staffName: p.staffName, sessions: 0, totalMs: 0, customers: new Set(), shown: 0 };
    row.sessions += 1;
...
function buildContentBreakdown(list, max = 6) {
  const counts = new Map();
  for (const p of list) {
...
function buildInterestBreakdown(list) {
  const counts = new Map();
  for (const p of list) {
 ...
 * presentations' customers went on to actually book — answers "which device
 * is winning us the most sales," not just "which device is used most."
...
function buildDeviceBreakdown(presentations, leadStatusById) {
  ...
  for (const p of presentations) {
    ...
    const row = byDevice.get(p.device) ?? { sessions: 0, bookedLeads: new Set() };
    row.sessions += 1;
...
function buildBlockFrequency(events) {
  const counts = new Map();
  for (const e of events) {
 ...
 * reason: <reason>". */
function buildBlockDetails(events) {
  const rows = [];
  for (const e of events) {
    if (e.type !== "status" || !e.label.includes(" blocked by ")) continue;
...
function buildTechnicalIssues(events) {
  const counts = new Map();
  for (const e of events) {
 ...
 */
function buildManagerComparison(list) {
 ...
 */
function buildManagerAccountability(events) {
  const byStaff = new Map();
  for (const e of events) {
...
function buildHourHistogram(list) {
...
function TrendChart({ data, accent }) {
  ...
  return (
    <div className={styles.chartCard} style={accent ? { "--chart-accent": accent } : undefined}>
 ...
 * as a response cue). */
function TopPropertiesChart({ data, accent }) {
  const max = Math.max(1, ...data.map((d) => d.count));
...
function PresentationsMiniTable({ rows }) {
 ...
 *
 * `onSelectStaff` jumps straight to that person's profile in Staff Activity —
 * otherwise this list is a dead end: a name with no way to act on it. */
function IdleStaffReport({ presentations, staffList, onSelectStaff }) {
...
function StaffLeaderboard({ rows }) {
  return (
...
function BusyHoursChart({ data, accent }) {
  const max = Math.max(1, ...data.map((d) => d.count));
...
function StyledDropdown({ value, placeholder, options, onChange }) {
...
function AddStaffModal({ managerList, onClose, onCreated }) {
 ...
 * any of them — and its first entry is what clicking it opens. The views
 * themselves are "customers", "logins", "staff", "analytics", "leads" and
 * "inventory".
 */
const VIEW_TABS = [
 ...
 * range inclusive. An empty object means "all time". */
...
function DateRangePicker({ range, onChange }) {
  ...
  const dayTs = (day) => new Date(month.getFullYear(), month.getMonth(), day).getTime();
 ...
 * until mounted so the very first paint can't show a time that is already
 * stale by the time the tick starts. */
 ...
 * per-customer table below and the staff panel's full-day view can reuse
 * the same list UI.
 *
 * `flat` is set when this sits inside another panel: it drops the card
 * chrome so there's no border-inside-a-border, and reclaims that padding for
 * the event list. */
function TimelinePanel({ panelKey, title, meta, timeline, onClose, flat = false }) {
...
const ROADMAP_EXCLUDE = new Set(["step"]);
 ...
 * records.
 */
function RoadmapDiagram({ timeline }) {
 ...
 *
 * `eventsLoading` is true until this staff member's activity has actually
 * come back, so the walkthrough list can say "loading" instead of "nothing
 * recorded yet" — which is a very different, and wrong, thing to tell a
 * manager. `viewer` is who's doing the blocking/kicking, attributed on the
 * reason log below.
 */
function StaffControlPanel({ staffEmail, staffName, events, eventsLoading, onClose, viewer }) {
  ...
  const logControlAction = (type, label) => {
    track({
  ...
  const toggleBlocked = async (slug, reason) => {
  ...
  const [blockReasonSlug, setBlockReasonSlug] = useState(null);
  ...
  const requestConfirm = (message, onConfirm) => setPendingConfirm({ message, onConfirm });
  ...
  const generalTimeline = useMemo(() => sorted.filter((e) => !e.leadId), [sorted]);
  const chips = useMemo(() => {
 ...
 */
function LeadsPanel({ viewer, staffList }) {
  ...
  function revealPhone(leadId, leadName) {
                ...
                <tr key={l.leadId}>
                  ...
                  <td>
                    <select
                      ...
                      onChange={(e) => changeStatus(l.leadId, e.target.value)}
                    >
                      {LEAD_STATUSES.map((s) => (
...
export function SessionReports({ brandLabel, title }) {
  ...
  const [leadsForDeviceReport, setLeadsForDeviceReport] = useState([]);
  useEffect(() => {
  ...
  const leadStatusById = useMemo(
    () => new Map(leadsForDeviceReport.map((l) => [l.leadId, l.leadStatus])),
  ...
  // self) — see server/routes/users.js.
