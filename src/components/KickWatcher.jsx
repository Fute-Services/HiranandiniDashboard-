...
/**
 * Mounted once in the app shell (src/App.jsx) so a force-logout (see
 * SessionReports's Sales Staff panel) takes effect on whichever page the
 * staff member is actually on — lead lookup, the Earth transition, or the
 * showcase — not just the showcase screen. Polls `/api/controls` (see
 * lib/controls.js for why this can't be localStorage: a manager and a
 * sales-staff member are on separate devices, so the "kicked" flag has to
 * live somewhere reachable from both, not in either one's own browser
 * storage).
 */
export function KickWatcher() {
   ...
   * ends when the page itself reloads (see lib/sign-out.js). */
