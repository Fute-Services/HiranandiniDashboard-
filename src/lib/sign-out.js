 ...
 * own:
 *
 * 1. The auth cookie is httpOnly, so only /api/logout can expire it, and the
 *    route guards (src/routes/guards.jsx) ask the server about exactly that
 *    cookie. Navigating before that response lands means the guard still sees
 *    a valid session and bounces /login back to the dashboard — the sign-out
 *    silently fails. clearSessionCookies now waits for it.
 *
 * 2. A hard navigation, not a router navigate(). The client-side variant
 *    lands on the same route it started from when that bounce happens, so the
 *    component never unmounts and its "signing out…" state has nothing to
 *    clear it — the button spins forever. A full load can't get stuck that
 *    way: the page that owns the state is gone either way. replace() rather
 *    than assign() so Back doesn't return to a dashboard the user just left.
 *
 ...
 */
export async function signOut(redirectTo = LOGIN_PATH) {
  try {
    logLogout();
