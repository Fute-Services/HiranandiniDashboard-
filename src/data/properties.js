 ...
 *
 * A `Property` is `{ slug, name, listName?, location, href, image?,
 * amenities? }`: `slug` is the stable key that will become the API/route key
 * once the backend lands, `name` is shown on the card face, `listName`
 * (defaulting to `name`) is shown in the footer list where entries read as a
 * set, `href` is the property's own external site (PRD §2), and `amenities`
 * is a generic list standing in for real per-project specs until the
 * CRM/content API lands (same mock-data pattern as leads.js/users.js).
 */
 ...
 * (sometimes) a set of projects inside it. Shape is `{ slug, name, location,
 * href, image?, amenities?, blurb?, projects }`, where `blurb` is the one
 * line under the name on the card and `projects` is empty for a portfolio
 * that is a single project.
 *
 ...
 */
export const showcaseProjects = portfolioGroups.map((g) => ({
  slug: g.slug,
  name: g.name,
  location: g.location,
  href: g.href,
...
export function projectsIn(slug) {
 ...
 *
 * Each entry carries a `portfolioSlug` alongside the ordinary property
 * fields, because the admin block list and the unit-availability grid are
 * both keyed by *portfolio* (see `showcaseProjects` — an admin blocks
 * "Fortune City", never "Elena" individually). The rail lists towers, so it
 * has to be able to ask "is my portfolio blocked?" — without this, blocking
 * Fortune City would leave all six of its towers still tappable on the rail.
 *
 ...
 */
export const railProjects = [...portfolioGroups]
